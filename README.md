...
while getattr(t, "kill", True):
```
Powyższy while pobiera wartość atrybutu `kill` swojego wątku i kontynuuje swoje działanie tak długo, jak jego wartość jest równa `True`. Wartość parametru `kill` zmienia główny wątek przy kończeniu działania projektu.
```python
sem.wait()
frame = inputQueue.get()
```
